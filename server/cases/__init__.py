...
def _load_case_from_db(case_slug: str) -> CaseData | None:
    ...
    # Fetch NPCs with their archetype
    npcs_rows = sb.table("npcs").select(
        "npc_slug, display_name, system_prompt, timeline, voice, voice_instruction, gender, archetype_id, archetypes(name)"
    ...
    # Fetch NPC evidence relevance and smoking gun map
    npc_db_rows = sb.table("npcs").select("id, npc_slug").eq("case_id", case_id_db).execute().data
    npc_db_id_to_slug = {row["id"]: row["npc_slug"] for row in npc_db_rows}
